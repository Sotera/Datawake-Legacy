 ...
 */
function setupListeners(worker) {
    try {

        worker.port.on("startLookaheadTimer", function (lookaheadTimerObject) {
        ...
        worker.port.on("setUrlRank", setUrlRank);
        worker.port.on("openExternalLink", openExternalTool);
        worker.port.on("markInvalid", markInvalid);
    } catch (e) {
...
function onToggle(state) {
    ...
    if (datawakeInfo != null && datawakeInfo.isDatawakeOn && constants.isValidUrl(tabs.activeTab.url)) {
        if (mainPanel != null || mainPanel != undefined)
            mainPanel.destroy();
        clearTimers();
        mainPanel = sidebar.Sidebar({
            id: "datawake-sidebar",
            title: 'Datawake Trail Entities',
            ...
            onAttach: function (worker) {
              worker.port.on("init", function () {
                  ...
                  getEntities(datawakeInfo.domain.name, function (entities) {
                      worker.port.emit("entities", entities);
                  });
                  service.getExternalLinks(function (externalLinks) {
                      worker.port.emit("externalLinks", externalLinks);
                  });
              });
...
function markInvalid(entity) {
    ...
    requestHelper.post(post_url, JSON.stringify(entity), function (response) {
        addon.port.emit("marked", entity.value);
    });
}
...
function emitRanks(datawakeInfo) {
    ...
    requestHelper.post(url, post_data, function (response) {
        ...
        addon.port.emit("ranking", rankingInfo);
    });
}
