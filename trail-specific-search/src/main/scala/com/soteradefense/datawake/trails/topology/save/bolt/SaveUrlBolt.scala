...
class SaveUrlBolt(sqlCredentials: SqlCredentials, insertUrlEntitySql: String, outputFields: Fields) extends SqlUpdateBolt(sqlCredentials) {
  override def execute(input: Tuple, collector: BasicOutputCollector): Unit = {
    ...
    val rank = input.getStringByField("sqlRank")
    val sqlWrapper = new SQLExecutor(connection)
    val rowsChanged = sqlWrapper.insertCount(insertUrlEntitySql, 0.0, org, domain, trail, url, title, rank)
    logger.info("PageRank: {}", rank)
    logger.info("URL: {}", url)
    logger.info("Emitting to Rank Count: {}", rowsChanged)
